...
里程碑 A、B 的代码、自动化测试和分 ABI Debug APK 构建已完成。人物数据只来自
当前 Emby 服务器，作品只查询电影和电视剧；媒体库位置提示继续使用服务端分页，
不加载完整媒体库。真实 Emby 服务器与 Android 设备验收尚待可用设备执行；
`# / A-Z` 导航仍未实施。
