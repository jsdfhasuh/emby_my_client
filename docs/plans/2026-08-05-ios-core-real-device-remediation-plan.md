...
### 8.8 Gate B 14.1.10 的窄幅安全诊断导出授权

为解决未登录设备所有者无法进入登录后诊断页的问题，本计划在仍停留于 Gate B、且不进入阶段 8 的前提下，明确授权增加以下窄幅能力：
...
Gate B `14.1.10` 的执行方式固定为：

1. 原始 `emby_client_diagnostics.log` 继续只保存在应用私有目录；
2. 不允许导出原始日志、`DiagnosticLog.path` 或应用沙盒目录；
3. 设备所有者只导出严格 allowlist 的安全诊断报告；
4. CI 自动测试继续验证通用 `DiagnosticLog` 写入和脱敏路径；
5. 真机验收验证导出的安全报告不含设备所有者本次实际输入的敏感值；
6. 不得把“安全报告通过”写成“完整原始日志已经上传或公开审计”。
